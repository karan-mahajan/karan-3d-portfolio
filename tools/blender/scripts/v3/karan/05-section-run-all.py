"""Run karan's FULL v3 world build — sections 01 through 05, everything.

The single master entry point. Rebuilds the whole world from zero into
`world-v3-karan.blend`, with the section-05 foliage layer LAST so it can read
the section-marker footprints placed in section 04 and avoid scattering into
the section clearings:
  ...
  Step 9: karan/05-foliage-*.py               (bushes, flowers, fences, bricks — LAST)
...
Bruno's own 03/04/05 placement scripts are intentionally skipped — nothing is
...
Inside Blender — Python Console (more reliable than Alt+P):
    exec(open('/Users/mahajankaran/Documents/Projects/karan-portfolio/tools/blender/scripts/v3/karan/05-section-run-all.py').read())

Headless:
    blender --background --factory-startup \
        --python tools/blender/scripts/v3/karan/05-section-run-all.py
"""
...
# karan/05 scripts `import foliage_common`; ensure KARAN_DIR is importable.
...
def _run_section(label, directory, prefix):
    ...
    if not files:
        sys.stderr.write(f"[05-section] WARN: no files matched {prefix!r} in {directory}\n")
        return 0
    print(f"[05-section] {label}: {len(files)} files")
    for f in files:
        print(f"  [{label}] {f}")
...
def main():
    ...
    try:
        ...
        print(f"[05-section] saved -> {BLEND_PATH}")
    except Exception as e:
        sys.stderr.write(f"[05-section] save failed: {e}\n")
    print(f"[05-section] DONE ({total} steps total)")
